...
$data = $_REQUEST;
$arFilterMailing = array("IBLOCK_ID"=>15, "GLOBAL_ACTIVE"=>"N");
if(isset($data["mailing_id"]) && $data["mailing_id"] != ""){
	$arFilterMailing["ID"] = $data["mailing_id"];
}
...
header('Content-Disposition: attachment;filename="report_mailing_'.$data["mailing_id"].'('.$thisDate.').xlsx"');
...
$ob = CIBlockElement::getList(array(),$arFilterMailing,false,false,array("ID", "IBLOCK_ID", "NAME","DETAIL_TEXT","PROPERTY_USERS","PROPERTY_RUBRIC"));
$numList = 0;
$arHeads = array(
	"ID Рассылки", 
	"Рассылка",
