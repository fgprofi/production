				...
				<form action="" class="report-mailing">
					<div class="rubricator-select">
                        <div class="rubricator-items">
                            <?$rubricCheckName = "Выберите рассылку";
                            foreach($arResult as $mailing):?>
                                <div data-value='<?=$mailing["ID"]?>' class="rubricator-items__option"><?=$mailing["NAME"]?></div>
                            <?endforeach;?>
                        </div>
					...
					afterShow: function(instance, current) {
						...
						intervalProgress = setInterval(function(){
							...
							if(dataClear == fullTime){
								...
								setTimeout(function(){
									$.get('/ajax/create_mailing_feed.php','mailing_id='+data, function(dataAjax){
										//console.log(dataAjax);
										//$(".data_res").html(dataAjax);
										document.location.href = '/ajax/create_mailing_feed.php?mailing_id='+data;
									});
									$.fancybox.close();
								}, 1000);
							}
				...
				afterShow: function(instance, current) {
					...
					intervalProgress = setInterval(function(){
						...
						if(dataClear == fullTime){
							clearInterval(intervalProgress);
							setTimeout(function(){
								document.location.href = '/ajax/create_mailing_feed_xlsx.php';
								$.fancybox.close();
							}, 1000);
						}
					}, timeProgress);
