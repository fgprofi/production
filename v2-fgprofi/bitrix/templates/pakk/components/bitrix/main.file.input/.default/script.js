		...
		d.prototype = {
			...
			onUploadDone : function(item, result) {
				...
				if (BX(node))
				{
					...
					var hrefImg = $(".add_file .file-input .webform-field-item-wrap a").attr("href");
					$(".cabinet-user__load-photo_box .cabinet-user__load-photo").css("background", "url("+hrefImg+")");
					$(".cabinet-user__load-photo_box .cabinet-user__load-photo").css("background-size", "cover");
					$(".cabinet-user__load-photo_box .cabinet-user__load-photo").css("background-repeat", "no-repeat");
					$(".cabinet-user__load-photo").addClass("check_logo");
					$(".cabinet-user__load-photo_box.no_logo").detach();
					$(".cabinet-user__load-photo .remove_photo").removeClass("hide");
				}
				else
				{
					this.onUploadError(item, result, this.agent);
				}
				// делаем сохранение фото
				$(".send-photo").click();
			},
