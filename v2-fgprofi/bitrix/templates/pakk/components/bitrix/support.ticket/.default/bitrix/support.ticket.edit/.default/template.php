...
if($ar = $rs->Fetch()) 
{
	$arResult["TICKET"]["FIELDS"] = $ar;
}
global $USER;
if($USER->GetID() == 1){
	//echo '<pre>'; print_r($arResult["TICKET"]["FIELDS"]); echo '</pre>';
}
?>
...
<table class="support-ticket-edit data-table">
	<?if(isAdministrator()):?>
		...
		<tr>
			<td>
			<?if($arResult["TICKET"]["FIELDS"]["UF_AUTOR_FIO"] != ""):?>
				<?=GetMessage("UF_AUTOR_FIO")?>: <?=$arResult["TICKET"]["FIELDS"]["UF_AUTOR_FIO"]?>
				<br />
			<?endif;?>
...
<table class="support-ticket-edit-form data-table">
	...
	<?if (empty($arResult["TICKET"])):?>
	<tr>
		<td class="field-name"><span class="starrequired">*</span><?=GetMessage("SUP_CATEGORY")?>:</td>
		<td>
			<?
	...
	<?else:?>
		<?if($arResult["TICKET"]["CATEGORY_ID"]):?>
	...
	<?if (strlen($arResult["TICKET"]["DATE_CLOSE"])<=0):?>
		<?if($_GET["ID"] != 0):?>
			...
			<tr class="tr-description">
				<td>Если вопрос вашего обращения решён, вы можете закрыть его выставив галочку</td>
			</tr>
		<?endif;?>
	<?else:?>
	<tr>
