...
$(document).ready(function(){
	$("form[name=support_edit]").on("submit", function(){
		$(this).find(".error_after_input").detach();
		var data = $(this).serializeArray();
		var errorCategory = false;
		for (var i = data.length - 1; i >= 0; i--) {
			if(data[i].name == "CATEGORY_ID" && data[i].value == ""){
				errorCategory = true;
			}
		}
		//console.log(data);
		if(errorCategory){
			if(!$(this).find(".error_after_input").length){
                $(this).prepend("<div class='error_after_input'>Необходимо выбрать категорию</div>");
            }
			//$.fancybox.open("<div>Необходимо выбрать категорию</div>");
			setTimeout(function(){
				$("form[name=support_edit] input[type=submit]").removeAttr("disabled");
			}, 300);
			return false;
		}
	});
...
function translit(objTextarea)
{
	...
	if (textbody)
	{
		if (TRANSLIT_way==0) // ëàòèíèöà -> êèðèëëèöà
		{
