...
foreach ($arResult['ITEMS'] as $item) {
    $key = FormatDate("d F", MakeTimeStamp($item["DATE_ACTIVE_FROM"]));
            ...
            <?foreach($groop as $item):?>
                <a id="item-<?=$item["ID"]?>" href="<?=$item["DETAIL_PAGE_URL"]?>" class="newsletter__item item-<?=$item["ID"]?>">
                    ...
                    <div class="newsletter-text">
                        ...
                        <?
                        $date_active_to = "";
                        if($item["DISPLAY_PROPERTIES"]["DATE_SEND"]["VALUE"]){
                            $date_active_to = $item["DISPLAY_PROPERTIES"]["DATE_SEND"]["VALUE"];
                        }?>
