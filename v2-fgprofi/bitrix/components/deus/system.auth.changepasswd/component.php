...
// делаем подмену изменения пароля для юрлица
if($_GET['USER_LOGIN'] > 0) {
	$curUser = CUser::GetByLogin($_REQUEST['USER_LOGIN'])->GetNext();

	if($curUser['CHECKWORD'] == $_REQUEST['USER_CHECKWORD'] && $_REQUEST['USER_PASSWORD'] == $_REQUEST['USER_CONFIRM_PASSWORD']){
		$res1 = $USER->Update($curUser['ID'], array('PASSWORD'=>$_REQUEST['USER_PASSWORD'], 'CONFIRM_PASSWORD'=>$_REQUEST['USER_CONFIRM_PASSWORD']));

		if($res1)
		{
			$arResult['ERROR_MESSEGE'] = '';
			$arParams["~AUTH_RESULT"]['MESSAGE'] = 'Пароль успешно изменен';
			$arParams["AUTH_RESULT"]['MESSAGE'] = 'Пароль успешно изменен';
			$arParams["~AUTH_RESULT"]['TYPE'] = 'OK';
			$arParams["AUTH_RESULT"]['TYPE'] = 'OK';
			unset($arParams["~AUTH_RESULT"]['FIELD']);
			unset($arParams["AUTH_RESULT"]['FIELD']);
			$arResult["SHOW_FORM"] = 0;

			$send_mess = new reestr\sendMessage();
			$main_config = new reestr\mainConfig();
			$emailStr = $main_config->FindFizByUr($_REQUEST['USER_LOGIN']);
			$send_mess->sendChangePassUL($curUser['NAME'],$emailStr);
		}
	}
}
